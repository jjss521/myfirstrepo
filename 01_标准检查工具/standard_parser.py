...
from models import StandardRef, MergedTextLine
...
def fix_ocr_digits(text: str) -> str:
    ...
    pattern = re.compile(
        r'([A-Z]{1,4}(?:/[A-Z])?\s+)([0-9A-Za-z]+)(?=\s*[-—－]\s*\d{4})'
    )
...
def extract_standards(lines: List[MergedTextLine],
                      source_file: str) -> List[StandardRef]:
    """
    从合并后的文本行中提取标准编号和名称。
    ...
    for ref in standards:
        ...
        if key not in seen:
            ...
            if ref.confidence > existing.confidence:
                ...
                existing.raw_text = ref.raw_text
            # 如果现有的名称为空而新的有名称，更新名称
            if not existing.name and ref.name:
...
def split_standard_entries(text: str) -> list:
    """按换行、分号、多空格灵活分割标准条目
