...
def parse_args():
    ...
    parser.add_argument(
        '--delay-min',
        type=float, default=REQUEST_DELAY_MIN,
        help=f'最小请求间隔秒数 (默认: {REQUEST_DELAY_MIN})',
    )
    parser.add_argument(
        '--delay-max',
        type=float, default=REQUEST_DELAY_MAX,
...
def save_cache(standards, output_dir):
    """将提取的标准保存为JSON缓存"""
...
def run_web_phase(standards, delay_min, delay_max):
    """网站查询阶段：逐条查询标准有效性"""
...
def main():
    ...
    if args.skip_web:
        ...
        validated = []
        from models import ValidatedStandard
        for ref in standards:
