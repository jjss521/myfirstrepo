...
def _compute_table(
    ...
    - 列: 左侧标签列 + 每个回路一列
    - 行: 各参数行
    - 列宽 = horizontal_spacing (与回路块间距一致，确保列中心对齐)
    """
    spacing = layout_cfg.horizontal_spacing
    col_width = spacing  # 列宽等于回路间距，使列中心与回路块中心对齐
    label_w = layout_cfg.table_label_col_width
    row_h = layout_cfg.table_row_height

    # 表格 X 起点: 标签列在第一个回路左侧，使第一列中心对齐 bus_x
    table_x = layout_cfg.bus_x - col_width / 2 - label_w
    # 表格顶部 Y: 母线 Y 下方一定距离
