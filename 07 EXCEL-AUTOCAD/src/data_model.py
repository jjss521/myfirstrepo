...
class TableLayout:
    ...
    col_width: float = 0      # 数据列宽
    label_col_width: float = 30  # 行标签列宽
...
class LayoutConfig:
    ...
    table_col_width: float = 80           # 数据列宽 (mm) [已弃用: 现在自动使用 horizontal_spacing]
...
class TitleBlockConfig:
    ...
    insert_at_x: float = 0
    insert_at_y: float = 0
    blocks: Dict[str, str] = field(default_factory=lambda: {"A1": "A1", "A2": "A2"})
    default_block: str = "A2"
