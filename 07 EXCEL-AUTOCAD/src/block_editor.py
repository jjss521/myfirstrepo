...
class BlockParams:
    ...
    # --- 图框 ---
    # A2 图框 (594×420mm 幅面使用)
    A2_TITLE_W = 180
    A2_TITLE_H = 50
    # A1 图框 (841×594mm 幅面使用)
    A1_TITLE_W = 250
    A1_TITLE_H = 60
...
class BlockEditor:
    ...
    def create_title_block(self) -> bool:
        ...
        在 title_block.dwg 中创建两个图块:
        - A1: 用于 A1 幅面 (841×594mm)
        - A2: 用于 A2 幅面 (594×420mm)
        ...
        try:
            ...
            self._build_title_block_def(
                "A2",
                BlockParams.A2_TITLE_W,
                BlockParams.A2_TITLE_H,
            )
            logger.info("标题栏图块 A2 已创建 (%.0f×%.0fmm)",
                        BlockParams.A2_TITLE_W, BlockParams.A2_TITLE_H)

            # 创建 A1 图框
            self._build_title_block_def(
                "A1",
                BlockParams.A1_TITLE_W,
                BlockParams.A1_TITLE_H,
            )
            logger.info("标题栏图块 A1 已创建 (%.0f×%.0fmm)",
                        BlockParams.A1_TITLE_W, BlockParams.A1_TITLE_H)
    ...
    def purge_blocks(self) -> int:
        ...
        count = 0
        all_names = [b["name"] for b in STANDARD_BLOCKS] + ["A1", "A2"]
    ...
    def validate_library(self) -> Dict:
        ...
        result = {
            "ok": True,
            "blocks": {},
            "title_blocks": {"A1": False, "A2": False},
            "missing_blocks": [],
        }
        ...
        for size in ["A1", "A2"]:
            try:
                self.doc.Blocks.Item(size)
                result["title_blocks"][size] = True
            except Exception:
                result["title_blocks"][size] = False
