...
class RulesPage(ctk.CTkFrame):
    """Knowledge base viewer / editor — rule items in a treeview with detail panel."""
    ...
    def __init__(self, master, ctx: EngineContext, **kwargs) -> None:
        ...
        self._sel_code = None
        self._sel_stage = None
        self._sel_cat = None
        self._sel_order = None
        self._build()
    ...
    def _build(self) -> None:
        ...
        # Toolbar above tree
        toolbar = ctk.CTkFrame(left_card, fg_color="transparent")
        toolbar.pack(fill="x", padx=10, pady=(10, 0))
        self.add_item_btn = ctk.CTkButton(
            toolbar, text="＋ 添加条目", font=get_ctk_font("caption"),
            fg_color=COLORS["accent_green"], text_color="white",
            corner_radius=8, width=100, command=self._add_item,
        )
        self.add_item_btn.pack(side="left")
        self.del_item_btn = ctk.CTkButton(
            toolbar, text="🗑 删除选中", font=get_ctk_font("caption"),
            fg_color=COLORS["bg_primary"], text_color=COLORS["text_primary"],
            hover_color="#e74c3c", corner_radius=8, width=100,
            command=self._delete_item,
        )
        self.del_item_btn.pack(side="left", padx=(6, 0))
        # Edit toolbar hidden by default
        self.add_item_btn.pack_forget()
        self.del_item_btn.pack_forget()
        ...
        self.detail_title = ctk.CTkLabel(
            self.browse_frame, text="选择一个条目查看详情", font=get_ctk_font("body"),
            text_color=COLORS["text_primary"],
        )
        ...
        # Switches row
        sw_frame = ctk.CTkFrame(self.edit_frame, fg_color="transparent")
    ...
    def _toggle_edit_mode(self) -> None:
        ...
        if self.edit_mode:
            ...
            self.add_item_btn.pack(before=self.del_item_btn)
            self.del_item_btn.pack(after=self.add_item_btn, padx=(6, 0))
    ...
    def _show_selected(self) -> None:
        """Show selected item in current mode (browse or edit)"""
        if not self._sel_code:
            return
        rule = self.ctx.engine.load_rule(self._sel_code)
        if not rule:
            return
        stage_data = rule.get("design_stages", {}).get(self._sel_stage, {})
        categories = stage_data.get("sections", stage_data) if isinstance(stage_data, dict) else {}
        cat_data = categories.get(self._sel_cat, {})
        for item in cat_data.get("items", []):
            if str(item.get("order", "")) == self._sel_order:
                if self.edit_mode:
                    ...
                    self.detail_title.configure(
                        text=f'{rule["project_type"]} → {cat_data.get("title", "")} → {item.get("title", "")}')
    ...
    def _on_select(self, event) -> None:
        ...
        if not tags or len(tags) < 4:
            return
        self._sel_code, self._sel_stage, self._sel_cat, self._sel_order = tags
        self._show_selected()

    # ── Edit operations ──

    def _save_changes(self) -> None:
        """Save edited item back to JSON"""
        if not all([self._sel_code, self._sel_stage, self._sel_cat, self._sel_order]):
            return
        rule = self.ctx.engine.load_rule(self._sel_code)
        if not rule:
            return
        ...
        for item in cat_data.get("items", []):
            if str(item.get("order", "")) == self._sel_order:
                # Update fields from edit widgets
                ...
                break
        if self.ctx.engine.save_rules_to_json(self._sel_code, rule):
            self.refresh()
            # Re-select
            for child in self.listbox.get_children():
                t = self.listbox.item(child, "tags")
                if t == (self._sel_code, self._sel_stage, self._sel_cat, self._sel_order):
                    self.listbox.selection_set(child)
                    break
            # Cannot call ctk message box; just refresh the view
        else:
            # Fallback indication
    ...
    def _add_item(self) -> None:
        ...
        if not rule:
            return
        if self._sel_stage and self._sel_cat:
            stage_data = rule.get("design_stages", {}).get(self._sel_stage, {})
            categories = stage_data.get("sections", stage_data) if isinstance(stage_data, dict) else {}
            cat_data = categories.get(self._sel_cat, {})
            if cat_data:
                ...
                if self.ctx.engine.save_rules_to_json(self._sel_code, rule):
                    self.refresh()
                    self._sel_order = str(max_order + 1)
    ...
    def _delete_item(self) -> None:
        ...
        before = len(items)
        cat_data["items"] = [it for it in items if str(it.get("order", "")) != self._sel_order]
