...
class BlockEditorWindow:
    ...
    def _build_block_list(self, parent):
        ...
        self.tree.insert("", tk.END, iid="_sep",
                         values=("── 标题栏 ──", ""))
        self.tree.insert("", tk.END, iid="TITLE_BLOCK",
                         values=("标题栏图块", "—"))
    ...
    def _on_tree_select(self, event):
        ...
        if name == "TITLE_BLOCK":
            self.detail_name_var.set("TITLE_BLOCK")
            self.detail_desc_var.set("标题栏图块")
    ...
    def _validate(self):
        ...
        try:
            ...
            title_status = "✓" if result["title_block"] else "✗"
            lines.append(f"\n  {title_status} TITLE_BLOCK (标题栏)")

            overall = "通过" if result["ok"] and result["title_block"] else "有问题"

            msg = (
                f"验证结果: {overall}\n\n"
                f"标准图块: {ok_count}/{len(STANDARD_BLOCKS)} 存在\n"
                f"标题栏: {'存在' if result['title_block'] else '不存在'}\n\n"
                + "\n".join(lines)
            )
            ...
            if result["ok"] and result["title_block"]:
