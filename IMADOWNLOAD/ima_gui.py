...
class macOSApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.stat_downloaded = [None]
        self.stat_failed = [None]
        self.stat_skipped = [None]
        
        # Button references (will be set in _create_widgets)
        self.scan_btn = None
    ...
    def _setup_fonts(self):
        ...
        pass  # Font family set per-widget using family=None for cross-platform compatibility

    def _create_font(self, size=13, weight="normal"):
        """Create a cross-platform font"""
        return ctk.CTkFont(size=size, weight=weight, family=None)
    ...
    def _create_widgets(self):
        ...
        ctk.CTkLabel(main, text="IMA Knowledge Base Downloader",
                      font=self._create_font(24, "bold"),
        ...
        ctk.CTkLabel(row2, text="Output Directory", font=self._create_font(12, "bold"),
                      text_color="#6E6E73").pack(anchor="w")
        
        dir_row = ctk.CTkFrame(row2, fg_color="transparent")
        dir_row.pack(fill="x", pady=(4, 0))
        
        self.dir_entry = ctk.CTkEntry(dir_row, height=32, font=self._create_font(13),
                                       fg_color="#F5F5F7", border_width=0, corner_radius=6)
        ...
        browse_btn = ctk.CTkButton(dir_row, text="Browse", width=72, height=32, 
                                    font=self._create_font(12, "bold"),
        ...
        for text, fg_color, hover_color, cmd, *rest in button_configs:
            ...
            btn = ctk.CTkButton(btn_frame, text=text, height=32, width=128,
                               font=self._create_font(13, "bold"),
            ...
            # Save button references for _set_state
            if text == "Scan":
                self.scan_btn = btn
            elif text == "Download Selected":
                self.download_btn = btn
            elif text == "Download All":
                self.download_all_btn = btn
            elif text == "Stop":
                self.stop_btn = btn
        ...
        for i, (label, value, color) in enumerate(stat_configs):
            ...
            # Save stat label references
            if label == "Total":
                self.stat_total = stat_widget
            elif label == "Downloaded":
                self.stat_downloaded = stat_widget
            elif label == "Failed":
                self.stat_failed = stat_widget
            elif label == "Skipped":
                self.stat_skipped = stat_widget
        ...
        for text, fg_color, hover_color in tree_btn_configs:
            btn = ctk.CTkButton(tree_btn_row, text=text, width=72, height=24, 
                               font=self._create_font(11, "bold"),
        ...
        self.log_text = ctk.CTkTextbox(log_frame, 
                                       font=ctk.CTkFont(size=11, family=None),
                                       fg_color="#F5F5F7", 
                                       text_color="#1D1D1F", 
                                       corner_radius=6,
                                       border_width=0)
    ...
    def _create_stat_box(self, parent, label, value, color):
        ...
        ctk.CTkLabel(frame, text=label, font=self._create_font(11, "bold"),
                      text_color="#8E8E93").pack()
        lbl = ctk.CTkLabel(frame, text=value, 
                          font=self._create_font(22, "bold"),
                          text_color=color)
        lbl.pack()
        return (lbl, frame)
    ...
    def _set_state(self, scanning=False, downloading=False):
        ...
        for btn in [self.scan_btn, self.download_btn, self.download_all_btn, self.stop_btn]:
            if btn is None:
                continue
            text = btn.cget("text")
            if scanning:
                if text == "Stop":
                    btn.configure(state="normal", fg_color="#FF3B30")
                else:
                    btn.configure(state="disabled", fg_color="#8E8E93")
            elif downloading:
                if text == "Stop":
                    btn.configure(state="normal", fg_color="#FF3B30")
                else:
                ...
                colors = {"Scan": "#007AFF", "Download Selected": "#34C759", 
                         "Download All": "#5856D6", "Stop": "#8E8E93"}
                btn.configure(state="normal" if text != "Stop" else "disabled", 
                            fg_color=colors.get(text, "#8E8E93"))
