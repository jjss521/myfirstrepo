...
def _lookup_ref_params(name):
    """根据设备名称查询Kx参考值，返回(kx, cos_phi, tan_phi)，无匹配时返回默认值(0.8, 0.8, 0.75)"""
    matches = KX_DB.fuzzy_search(name, top_n=1)
    if matches:
        best = matches[0]
        return best.kx_avg, best.cos_phi_avg, best.tan_phi_avg
    return 0.8, 0.8, 0.75
...
def _append_default_equip(equip_list):
    """为每个导入的建/构筑物自动新增照明和自控仪表两项默认设备

    照明: 3kW, 自控仪表: 2kW。需要系数和功率因数从参考值表查询，
    无匹配时取默认值0.8。
    """
    defaults = [
        ("照明", 3.0),
    ...
    for name, power in defaults:
        kx, cos_phi, tan_phi = _lookup_ref_params(name)
...
def parse_excel_file(file_path):
    ...
    for row in data_rows:
        ...
        # 正则未提取到有效功率时，尝试阀门功率推断
        if power is None or power <= 0:
            from .valve_power_map import infer_valve_power
            spec_text = str(row[spec_c]).strip() if spec_c is not None and spec_c < len(row) else ""
            power = infer_valve_power(name, spec_text)
    ...
    # 自动新增两项默认设备（照明3kW + 自控仪表2kW）
    _append_default_equip(equip_list)
